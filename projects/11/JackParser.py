...
from JackSymbolTable import ClassSymbolTable
...
class JackClassVariableDeclaration:
    ...
    def populate_class_symtable(self, class_symtable):
        kind = self.tokens[0].value
        type_ = self.tokens[1].value
        for token in self.tokens[2:-1]:
            if token.value == ',':
                continue
            class_symtable.addSymbol(token.value, type_, kind)
...
class JackClassVariablesDeclaration:
    ...
    def populate_class_symtable(self, class_symtable):
        for var in self.variables:
            var.populate_class_symtable(class_symtable)
...
""".format(self.class_keyword.serialize(), self.class_name.serialize(), self.open_bracket.serialize(),\
    ...
    def compile(self, vm_writer):
        class_symtable = ClassSymbolTable(self.class_name.value)
        self.class_var_decs.populate_class_symtable(class_symtable)
        self.subroutine_decs.populate_class_symtable(class_symtable)
        self.subroutine_decs.compile(vm_writer, class_symtable)
        print(class_symtable)
